                ...
                if (hit.collider.gameObject) // TODO: maybe change this to "if hits terrain" not just any gameObject
                {
                    float slopeAngle =
                        ...
                        Vector3.Angle(Vector3.up,
                            hit.normal); // get the angle between the up vector and the object the ray hits

                    float radius = Mathf.Abs(0 / Mathf.Sin(slopeAngle));

                    if (slopeAngle >= 45f /*change for different angle*/ * Mathf.Deg2Rad)
                    {
                        if (hit.distance - (0.5f - playerCameraTransform.position.z) >
                            Mathf.Abs(Mathf.Cos(slopeAngle) * radius) + 0.01
                        ) // 0.01 is a threshhold to prevent some bugs
                        {
                            return true;
                        }
