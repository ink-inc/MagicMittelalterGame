...
public class Interactable_PlayerMaxLife : Interactable
{
    ...
    public override void interact()
    {
        if (playerProp.GetMaxHealth() + value > 0)  //players max health should always be below zero
        {
            ...
            playerProp.SetHealth(Mathf.Clamp(playerProp.GetCurHealth(), 0, playerProp.GetMaxHealth())); //reloads player current life if max life has been decreased below current life
        }
    }
}
