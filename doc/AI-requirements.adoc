...
So therefor the _game code_ should provide information sharing methods. E.g. give me the next `n` closest enemies. The
return value should be a list containing all the possible information about the five nearest enemies. So that than the 
agent can ask for example: "Whats your health status enemy 1?".
...
_This part needs some technically discussion and will later be written._